...
def test_value_conversion():
    ...
    for input_val, expected in test_cases:
        ...
        assert type(result) == type(expected), f"Type mismatch for {input_val}: got {type(result)}, expected {type(expected)}"
...
def test_string_edge_cases():
    ...
    test_cases = [
        ...
        ("\"Very long string " + "x"*100 + "\"", "Very long string " + "x"*100),  # Long string (shortened to avoid test slowness)
    ]
...
def test_complex_smt_output_extraction():
    ...
    for var, type_str, val_str in matches:
        ...
        if var in expected_values:
            ...
            if isinstance(expected_values[var], float):
                ...
                assert abs(val - expected_values[var]) < 1e-10, f"Failed for variable {var}: got {val}, expected {expected_values[var]}"
            else:
                assert val == expected_values[var], f"Failed for variable {var}: got {val}, expected {expected_values[var]}"
            assert type(val) == type(expected_values[var]), f"Type mismatch for {var}: got {type(val)}, expected {type(expected_values[var])}"
    ...
    assert tested_vars == set(expected_values.keys()), f"Not all variables were tested: missing {set(expected_values.keys()) - tested_vars}"
