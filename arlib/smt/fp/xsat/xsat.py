...
def mcmc(args,i):
    ...
    res= op.basinhopping(lambda X: R_quick(X,i,foo.R),sp,niter=200*len(sp),stepsize=args.stepSize, minimizer_kwargs=_minimizer_kwargs,callback=_callback_global)
    if args.showResult:
    ...
    if res.fun!=0 and res.fun<args.round2_threshold:
        ...
        res_round2 = op.basinhopping(obj_near,np.zeros(foo.dim),niter=200*len(np.zeros(foo.dim)),stepsize=args.round2_stepsize,minimizer_kwargs=_minimizer_kwargs,callback=_callback_global)
...
def mcmc_round1(args):
    ...
    res=op.basinhopping(obj,sp,niter=200*len(sp),stepsize=args.stepSize,minimizer_kwargs={'method':args.method},callback=_callback_global)
    if args.showResult:
...
def mcmc_round2(args,X_star):
    ...
    for (scale,scale_inv) in scales():
        res = op.basinhopping(lambda x: foo_ulp.R(* scale(x)),scale_inv(X_star),niter=200*len(scale_inv(X_star)) ,minimizer_kwargs={'method':args.method},callback=_callback_global,stepsize=args.round2_stepsize)
        if res.fun<R_star:
...
def mcmc_round3(args,X_star):
    ...
    res = op.basinhopping(obj_near,np.zeros(foo_ulp.dim),niter=200*len(np.zeros(foo_ulp.dim)),minimizer_kwargs={'method':args.method},callback=_callback_global,stepsize=args.round3_stepsize)
...
def run_mcmc_single(args):
    ...
    if args.showResult:
        ...
        res_round2 = op.basinhopping(obj_near,np.zeros(foo_ulp.dim),niter=200*len(np.zeros(foo_ulp.dim)),stepsize=100.0,minimizer_kwargs={'method':args.method},callback=_callback_global)
        if args.showResult:
...
#    parser.add_argument('smt2_file', help='specify the smt2 file to analyze.',type=argparse.FileType('r'))
    ...
    parser.add_argument('--command_compilation', help='the command used to compile the generated foo.c to foo.so', default='gcc -O3 -fbracket-depth=2048 -fPIC -I /usr/local/Cellar/python/3.13.7/Frameworks/Python.framework/Versions/3.13/include/python3.13/ %(file)s.c -dynamiclib -o %(file)s.so -L /usr/local/Cellar/python/3.13.7/Frameworks/Python.framework/Versions/Current/lib/ -lpython3.13')
...
#    parser.add_argument("--single", help="single processor  (default: true)",default=True,action='store')
    ...
    if not args.multi:
        ...
        def log_result(result):
            ...
            print("[Xsat-multi] ENTERING: ",mp.current_process().name,  "log_result Minimum=", R_star)
