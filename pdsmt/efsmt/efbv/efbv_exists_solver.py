...
class ExistsSolver(object):
    def __init__(self, manager: EFBVFormulaManager):
        ...
        self.solver = Solver(name=self.solver_name)
        ...
        if num == 1:
            if self.solver.solve():
                ...
                for val in model:  # project?
                    # TODO: can we just take the first-k values of the model as existential_model?
                    #  assuming that the universal bools are always at the beginning
                    if abs(val) in self.fml_manager.existential_bools:
